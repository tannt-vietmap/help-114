...
//add base map
L.tileLayer(`https://maps.vietmap.vn/api/tm/{z}/{x}/{y}@2x.png?apikey=${apiKey}`, {
    maxZoom: 19,
}).addTo(map);
...
const processChange = debounce(() => {
  ...
  const search = async (text) => {
      ...
      const response = await fetch(
          `https://maps.vietmap.vn/api/search/v3?apikey=${apiKey}&text=${text}&layers=ADDRESS`
      );
      const myJson = await response.json();
  
      //display search
      getPlace( myJson[0].ref_id);
  };
